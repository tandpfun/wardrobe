...
import { ArrowCounterClockwise, Check, Images, Plus, SpinnerGap, Trash, UploadSimple, WarningCircle, X } from "@phosphor-icons/react";
import { ImmichPicker } from "./ImmichPicker.jsx";
...
export function WardrobeImportFlow({ onGarmentApproved, onModeledApproved }) {
  ...
  const [immich, setImmich] = useState(null);
  const [immichMode, setImmichMode] = useState(null);

  useEffect(() => {
    api(CONFIG_API).then(setSetup).catch((requestError) => setSetup({ ready: false, error: requestError.message }));
    api("/api/immich/config").then(setImmich).catch((requestError) => setImmich({ ready: false, error: requestError.message }));
    api(API)
      .then((storedJobs) => {
  ...
  const openImmich = (mode) => {
    setError("");
    setImmichMode(mode);
    setOpen(true);
  };

  const importImmichAssets = async (assets) => {
    const files = [];
    for (const asset of assets) {
      const response = await fetch(asset.imageUrl);
      if (!response.ok) throw new Error(`Could not retrieve ${asset.originalFileName || "that Immich photo"}.`);
      const blob = await response.blob();
      const filename = (asset.originalFileName || `${asset.id}.jpg`).replace(/\.[^.]+$/, ".jpg");
  ...
  const referenceSaved = async () => {
    const nextSetup = await api(CONFIG_API);
    setSetup(nextSetup);
    setImmichMode(null);
    setNotice({ tone: "complete", text: "Reference photo connected", detail: "Immich is ready. Choose outfit photos to start building your wardrobe." });
  };
  ...
  const activeStatus = setupRequired ? { tone: "error", text: !setup?.hasModelReference && immich?.ready ? "Choose reference from Immich" : "Setup required" } : active ? deriveStatus(active) : notice;
  ...
  return (
    <>
      ...
      <aside className={`import-tray${hasImportActivity ? " is-expanded" : ""}`} aria-label="Wardrobe imports">
        <button className="import-tray__button" type="button" onClick={() => setupRequired && !setup?.hasModelReference && immich?.ready ? openImmich("reference") : setupRequired || hasImportActivity ? setOpen(true) : inputRef.current?.click()} aria-label={setupRequired && immich?.ready ? "Choose reference from Immich" : setupRequired ? "Open setup instructions" : hasImportActivity ? "Open import progress" : "Add clothes"}>{activeStatus?.tone === "processing" ? <SpinnerGap size={19} className="import-spinner" /> : activeStatus?.tone === "error" ? <WarningCircle size={19} /> : readyCount ? <span>{readyCount}</span> : notice ? <X size={18} /> : <Plus size={19} />}</button>
        <div className="import-tray__actions">{active && <img className="import-tray__preview" src={active.stages?.garment?.assetUrl || active.stages?.garment?.failedAssetUrl || active.stages?.crop?.assetUrl || active.originalAssetUrl} alt="" />}<button className="import-tray__label" type="button" onClick={() => setupRequired && !setup?.hasModelReference && immich?.ready ? openImmich("reference") : setOpen(true)}>{activeStatus?.text || "Add clothes"}</button>{!setupRequired && <button className="import-icon-button" type="button" onClick={() => inputRef.current?.click()} aria-label="Choose images"><UploadSimple size={17} /></button>}</div>
      </aside>
        ...
        <section className="import-popover" role="dialog" aria-modal="true" aria-labelledby="import-title">
          ...
          {immichMode ? <ImmichPicker mode={immichMode} onClose={() => setImmichMode(null)} onImport={importImmichAssets} onReferenceSaved={referenceSaved} /> : !jobs.length ? setupRequired ? <div className="import-drop-target import-setup-warning"><WarningCircle size={30} /><h2>Setup required</h2><p>{!setup.hasApiKey && <>Add an <code>OPENAI_API_KEY</code> to your environment. </>}{!setup.hasModelReference && <>Choose a PNG reference photo of yourself. </>}</p>{!setup.hasModelReference && immich?.ready && <div className="import-source-actions"><button className="import-button import-button--primary" onClick={() => openImmich("reference")}><Images size={15} /> Choose from Immich</button></div>}</div> : <div className="import-drop-target"><UploadSimple size={28} /><h2>{notice ? "Choose your next source" : "Choose, paste, or browse Immich"}</h2><p>{notice?.detail || "We’ll isolate each clothing item, suggest its details, and hold everything for your approval."}</p><div className="import-source-actions"><button className="import-button import-button--primary" disabled={!setup?.ready} onClick={() => { setNotice(null); inputRef.current?.click(); }}>Choose images</button>{immich?.ready && <button className="import-button" onClick={() => openImmich("import")}><Images size={15} /> Browse Immich · {immich?.years || 4} years</button>}</div></div> : (
            <>
              ...
              <div className="import-actions"><button className="import-button" onClick={() => inputRef.current?.click()}><Plus size={14} /> Add another</button>{immich?.ready && <button className="import-button" onClick={() => openImmich("import")}><Images size={14} /> Add from Immich</button>}</div>
            </>
          )}
